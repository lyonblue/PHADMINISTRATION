...
const transporter = nodemailer_1.default.createTransport({
    ...
    secure: env_1.env.smtpPort === 465, // true para 465 (SSL), false para otros puertos (TLS)
...
async function sendEmail(opts) {
    ...
    if (!env_1.env.smtpUser || !env_1.env.smtpPass || env_1.env.smtpUser === 'tu_email@gmail.com' || env_1.env.smtpPass.includes('contraseña') || env_1.env.smtpPass.includes('abcd')) {
        if (env_1.env.nodeEnv !== 'production') {
            console.log('\n' + '='.repeat(70));
            console.log('📧 EMAIL DEV MODE (SMTP no configurado correctamente)');
            console.log('='.repeat(70));
            console.log('SMTP_USER:', env_1.env.smtpUser || '(vacío)');
            console.log('SMTP_PASS:', env_1.env.smtpPass ? '(configurado)' : '(vacío)');
    ...
    try {
        await transporter.sendMail({
            from: `"PH PTY Administration" <${env_1.env.emailFrom}>`,
            to: opts.to,
 ...
 */
function getProposalEmailHtml(data) {
    return `
<!DOCTYPE html>
<html>
<head>
...
async function sendProposalEmail(data) {
    ...
    const recipientEmail = env_1.env.proposalEmail;
    await sendEmail({
        to: recipientEmail,
